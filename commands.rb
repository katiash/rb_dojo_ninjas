...
# 6.3 Try to create a few more dojos but without specifying some of the required fields. 
# Make sure that the validation works and generates the errors.
dojo = Dojo.new(name: "CodingDojo Los Angeles", city: "Los Angeles")
dojo.save
dojo.errors.full_messages # => ["State can't be blank", "State is the wrong length 
...
# 7. Make sure you understand how to get all of the ninjas for any dojo (e.g. get all the ninjas 
# for the first dojo, second dojo, etc)
Dojo.first.ninjas
Dojo.second.ninjas
Dojo.third.ninjas

# 8. How would you only retrieve the first_name of the ninja that belongs to the second dojo and order 
# the result by created_at DESC order?  
Ninja.where(dojo: Dojo.second).select(:id, :first_name).order(created_at: :desc)

# 9. Delete the second dojo. How could you adjust your model so that it automatically 
# removes all of the ninjas associated with that dojo?
